...
조사에서 나온 여덟 가지를 전부 구현했다.

| # | 한 일 | 결과 |
|---|---|---|
| 1 | Ollama `keep_alive: 30m` | ✅ 5분 후 재적재로 타임아웃 나던 것을 막았다 |
| 2 | 판정기 게이트 | ✅ **이미 만족돼 있었다** — 판정기는 `gate()`가 ACT를 낸 뒤에만 돈다. 없는 것을 만들지 않았다 |
| 3 | 정지 명령 | ✅ `speak()`를 비블로킹으로 바꿔 말하는 중에도 듣는다 |
| 4 | 호출어 퍼지 매칭 | ✅ **자모 분해**로. 음절 단위는 뭉툭해서 "자비스"/"자비수"가 0.667이다 |
| 5 | 에코 처리 | ✅ 방향을 바꿨다 — 아래 참고 |
| 6 | `partial_ratio` | ✅ 손으로 만든 접두사 비교를 대체 |
| 7 | Mem0식 2단계 자동 기억 | ✅ Outbox로 비동기. 작은 모델만 쓴다 |
| 8 | MCP resources + prompts | ✅ 실제 클라이언트로 왕복 검증 |
| + | 민감정보 필터 | ✅ Trace와 기억에 들어가기 전에 지운다 |

### 5번은 그들과 다르게 갔다

그들은 에코 **부분만 잘라내고** 사용자 말을 살린다(`salvage_after_echo_tail`).
우리는 그러지 않기로 했다.

대신 **말하는 동안에는 이름을 부르거나 정지어를 말해야** 통과시킨다. 후속
발화 창이 그때는 통하지 않는다.

이유: 창이 열린 채 자기 목소리가 돌아오면 에코 필터를 **한 번만** 빠져나가도
무한 루프가 된다. 실제로 그랬고, 그때 브리핑이 14번 반복되며 Trace를 남겼다.
잘라내기는 필터를 정교하게 만들어 그 위험을 안고 가는 방식이고, 우리 규칙은
위험 자체를 없앤다. 끼어들 때 이름을 한 번 부르는 비용이 그보다 싸다.

잘라내기는 이 규칙으로 불편함이 남을 때 다시 본다.
...
다만 이건 voice feature를 다시 쓰는 일이다. 위의 것들을 먼저 넣었으니,
며칠 써 보고 그래도 남는 불편이 있는지 보고 정하는 것이 맞다.

**Open Interpreter 어댑터.** `interpreter exec`가 `claude -p` 와 같은 계약이라
`ClaudeCodeAdapter`를 `CliAgentAdapter`로 일반화할 수 있다(§2). 지금은 Claude가
잘 되고 있어 두 번째 CLI를 붙일 실제 이유가 없다. 필요해지면 하루면 된다.

**MCP sampling.** JUNVIS가 Claude Code 안에서 MCP 서버로 돌 때는 클라이언트의
모델을 빌려 쓸 수 있다 — `claude -p` 프로세스 기동 비용이 0이 된다. 다만
`junvis listen` 처럼 독립 실행할 때는 클라이언트가 없어 해당이 없다. **두 경로가
다르다**는 것을 알고 설계해야 해서 아직 넣지 않았다.
